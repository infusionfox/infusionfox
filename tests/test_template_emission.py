...
class TestWorkedExampleTemplates:
    ...
    def test_insulin_dextrose_emits_two_templates(self):
        """Insulin + dextrose."""
        r = client.post(
            "/insulin-dextrose-hyperK/compute",
            data={
                "weight_value": "20",
                "weight_unit": "kg",
            },
    ...
    def test_templates_live_inside_result_panel(self):
        ...
        assert template_pos > panel_start, "worked-example template not inside #result-panel"
...
class TestPickerCheckboxCounts:
    ...
    def _count_checkboxes(self, html, input_name):
        return len(re.findall(r'<input[^>]*name="' + re.escape(input_name) + r'"', html))
    ...
    def test_induction_checkbox_count(self):
        ...
        for species, weight in (("dog", "20"), ("cat", "4")):
            ...
            assert count == 2, f"expected 2 {species} induction checkboxes, got {count}"
    ...
    def test_picker_default_state_all_checked(self):
        ...
        checkbox_pattern = re.compile(r'<input[^>]*name="sel_opioid"[^>]*>')
        matches = checkbox_pattern.findall(r.text)
        assert len(matches) == 4
        for tag in matches:
            assert "checked" in tag, f"expected sel_opioid checkbox to be checked: {tag}"
